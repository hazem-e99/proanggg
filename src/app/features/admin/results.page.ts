...
export class AdminResultsPage implements OnInit {
  ...
  async loadResults() {
    try {
      ...
      const [submissions, examsRaw, studentsRaw] = await Promise.all([
        this.http.get<any[]>(`${this.apiUrl}/submissions`).toPromise(),
        this.http.get<any[]>(`${this.apiUrl}/exams`).toPromise(),
      ...
      // تجهيز النتائج
      this.results = await Promise.all(submissions.map(async (sub: any) => {
        ...
        return {
          ...
          score,
          totalQuestions,
          totalMarks: exam && exam.totalMarks ? exam.totalMarks : 0,
          correctAnswers,
          wrongAnswers,
          unansweredQuestions,
          passed,
      ...
      // جلب الامتحانات للفلاتر
      this.exams = exams;
      // تحديث الداتا في الشارت
