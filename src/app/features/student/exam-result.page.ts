  ...
  loadExamResult() {
    ...
    this.http.get<any>(`${this.apiUrl}/exams/${this.examId}`).pipe(
      switchMap(exam => {
        ...
        return this.http.get<any[]>(`${this.apiUrl}/submissions?examId=${this.examId}&_sort=submittedAt&_order=desc&_limit=1`);
      }),
      map(submissions => {
          ...
          map(questions => {
            ...
            const questionsWithAnswers = questions.map(q => {
              ...
              if (userAnswer && userAnswer.selectedAnswer) {
                // Convert 1-based user answer to 0-based index for comparison
                const userAnswerIndex = parseInt(userAnswer.selectedAnswer) - 1;
                isCorrect = userAnswerIndex === q.correctOption;
                
                console.log('Answer comparison:', {
                  selected: q.options[userAnswerIndex],
              ...
              return {
                ...q,
                userAnswer: userAnswer?.selectedAnswer ? q.options[parseInt(userAnswer.selectedAnswer) - 1] : '',
                isCorrect,
            ...
            const passingMarks = this.exam.passingMarks || 50; // Default to 50% if not specified
